package net.cnki.elasticsearch.template.service.impl;
...
//    private Logger log = LoggerFactory.getLogger(getClass());
    ...
    private QueryBuilder createQueryBuilder(String keyword, String... fieldNames){
        // 构造查询条件,使用标准分词器.短语精确：termQuery，标准：multiMatchQuery matchPhraseQuery
...
//                for(Text text : v.getFragments()) hight += text.string();
                ...
                String[] hight1 = new String[v.getFragments().length];
                for (int i = 0; i < v.getFragments().length; i++) {
//                	hight += v.getFragments()[i].string();
                	hight1[i] = v.getFragments()[i].string();
                	
				}
                hitMap.put(v.getName(),hight1);
            });
